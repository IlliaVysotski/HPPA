	...
	for (int i = 0; i < width; i++)
		for (int j = 0; j < height; j++)
			if (a[i + j * width] != b[i + j * width]) {
				//printf("\n[%d]: %d\t%d", i + j * width, (int)a[i + j * width], (int)b[i + j * width]);
				return false;
			}
	return true;
}
...
int main() {
	...
	const char srcImage[] = "C:\\Illia\\6 семестр\\АВП\\LR5-6\\Lab 5\\image.pgm"; 
	const char imageCPU[] = "C:\\Illia\\6 семестр\\АВП\\LR5-6\\Lab 5\\imageCPU2.pgm";
	const char imageGPU[] = "C:\\Illia\\6 семестр\\АВП\\LR5-6\\Lab 5\\imageGPU2.pgm";
